...
def get_collator(tokenizer, max_length: int, use_translations=True):
    ...
    def collate_fn(batch):
        ...
        if "translation" in batch and use_translations:
            for i, (lang, ex) in enumerate(
                zip(batch["language"], batch["translation"])
            ):
                inputs[i] = (
                    inputs[i]
                    + prompt2.format(
                        transl_lang="English" if lang != "usp" else "Spanish"
                    )
                    + _reassemble(ex)
                )
...
def create_trainer(
    ...
    trainer = transformers.Trainer(
        ...
        train_dataset=dataset["augmented"] if dataset else None,
        eval_dataset=None,
...
def main(
    ...
    if mode == "train":
        ...
        dataset = datasets.load_dataset(data_root, "all")
        collator = get_collator(tokenizer, max_length=MODEL_INPUT_LENGTH)
        dataset["augmented"] = dataset["augmented"].shuffle()
        dataset["augmented"] = dataset["augmented"].map(
            collator, batched=True, remove_columns=dataset["augmented"].column_names
        )
