# taken from baseline/src/data.py, added prediction field
import re
...
class IGTLine:
    ...
    def __init__(
        ...
        prediction: Optional[str],  # added for interleave_preds.py
    ):
    ...
    def seg_list(self, segmented=False) -> Optional[List[str]]:
        """Returns the gloss line of the IGT as a list.
        :param segmented: If True, will return each morpheme as a separate item.
        """
        if self.segmentation is None:
            return []
        if not segmented:
            return self.segmentation.split()
        else:
            return re.split(r"\s|-", self.segmentation)
    ...
    def set_pred(self, pred: str):
        self.prediction = pred

    def interleave_str(self):
        return (
            f"Trnsc:\t{self.transcription}\n"
            f"Segm:\t{self.segmentation}\n"
            f"Gloss:\t{self.glosses}\n"
    ...
    with open(path) as file:
        ...
        for line in file:
            ...
            if line_prefix == "\\t" and current_entry[0] is None:
                ...
                continue  # print("Skipping line: ", line)
            else:
