...
def _get_data_url(shortname, split):
    longname = _ALL_LANGS[shortname]
    if shortname == "lez":
        longname += "-romanized"
    if split == "augmented":
        return f"{longname}/{shortname}-train-artificial"
    else:
        return f"{longname}/{shortname}-{split}-track2-uncovered"
...
class Gloss(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        if self.config.name == "all":
            data_urls = {
                split: [_get_data_url(lang, split) for lang in _ALL_LANGS]
                for split in splits
            }
        else:
            data_urls = {
                split: [_get_data_url(self.config.name, split)] for split in splits
            }
        ...
        return [
            ...
            datasets.SplitGenerator(
                name=datasets.splits.NamedSplit("augmented"),
                gen_kwargs={
                    "text_paths": text_paths.get("augmented"),
                },
            ),
        ]
